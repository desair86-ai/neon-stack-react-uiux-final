...
export function ConfiguratorExperience({type="custom_neon"}){
  ...
  useEffect(()=>{const box=previewRef.current;if(!box)return;const fit=()=>{const probe=document.createElement("span"),cs=textRef.current?getComputedStyle(textRef.current):null;
   ...
   probe.textContent=text||"Preview";document.body.appendChild(probe);const sIdx=options?.sizes?.findIndex(s=>s.id===size?.id);const sizeRatios=[0.75,1.0,1.25,1.5];const ratio=(sIdx!==undefined&&sIdx>=0&&sIdx<sizeRatios.length)?sizeRatios[sIdx]:(physicalHeight(size)?physicalHeight(size)/13:1.0);const lines=String(text||"").split("\n").length;const baseMaxW=Math.max(120,box.clientWidth*0.62);const baseMaxH=Math.max(60,box.clientHeight*0.38/lines);let low=10,high=220;for(let i=0;i<18;i++){const mid=(low+high)/2;probe.style.fontSize=`${mid}px`;if(probe.scrollWidth<=baseMaxW&&probe.scrollHeight<=baseMaxH)low=mid;else high=mid}const scaledSize=Math.round(low*ratio);const maxCanvasW=box.clientWidth*0.90;const maxCanvasH=box.clientHeight*0.65/lines;probe.style.fontSize=`${scaledSize}px`;let finalSize=scaledSize;if(probe.scrollWidth>maxCanvasW||probe.scrollHeight>maxCanvasH){let sLow=10,sHigh=scaledSize;for(let j=0;j<12;j++){const sMid=(sLow+sHigh)/2;probe.style.fontSize=`${sMid}px`;if(probe.scrollWidth<=maxCanvasW&&probe.scrollHeight<=maxCanvasH)sLow=sMid;else sHigh=sMid}finalSize=Math.round(sLow)}setFontSize(Math.max(12,finalSize));document.body.removeChild(probe)};fit();const ro=new ResizeObserver(fit);ro.observe(box);return()=>ro.disconnect()},[text,font,size,shapes,options]);
