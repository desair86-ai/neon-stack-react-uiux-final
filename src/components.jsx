...
const defaultProducts = [
  ['Gaming Controller','Gaming','/images/1.png','BESTSELLER','1,499'],['Astronaut On Moon','Astronaut & Space','/images/astro_with_full_moon.png','MOJO MIX','1,999'],
...
function ProductCard({p}){return <Link className="productCard" href="/collections"><div className="productImg" style={{backgroundImage:`url(${p[2]})`}}><span className="badge">{p[3]||' '}</span><button onClick={e=>e.preventDefault()}><Heart/></button></div><div className="productInfo"><h3>{p[0]}</h3><small>{p[1]}</small><div><b>From ₹{p[4]}</b><span><ShoppingCart/></span></div></div></Link>}
